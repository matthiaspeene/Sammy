...
juce::AudioProcessorValueTreeState::ParameterLayout SammyAudioProcessor::createParameters()
{
                                        ...
                                        float defaultValue)
    {
        parameters.push_back(std::make_unique<AudioParameterFloat>(
            ...
            name,
            juce::NormalisableRange<float> { minimum, maximum },
            defaultValue));
    };

    addFloat("ATTACK", "Attack", 0.0f, 20.0f, 0.012f);
    addFloat("DECAY", "Decay", 0.0f, 12.0f, 1.5f);
    addFloat("SUSTAIN", "Sustain", 0.0f, 1.0f, 1.0f);
    addFloat("RELEASE", "Release", 0.0f, 20.0f, 0.012f);
    addFloat("START", "Start Position", 0.0f, 100.0f, 0.0f);
    addFloat("RANDOMS", "Start Position Randomization", 0.0f, 100.0f, 0.0f);
    addFloat("PITCH OFFSET", "Pitch Offset", -36.0f, 36.0f, 0.0f);
